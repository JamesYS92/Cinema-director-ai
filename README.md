...
| 변수명 | 설명 |
|--------|------|
| `STUDY_LLM_API_TOKEN` | **권장** — 스터디 포털 `study_live_...` 프록시 토큰 (일 300회) |
| `STUDY_LLM_API_URL` | (선택) 기본값 `https://vibe-llm-proxy-17280846291.asia-northeast3.run.app` |
| `STUDY_LLM_MODEL` | (선택) 기본값 `gemini-2.0-flash-lite` |
| `GEMINI_API_KEY` | 대안 — Google AI Studio 직접 키 (무료 일 20회 제한) |
| `YOUTUBE_API_KEY` | [YouTube Data API](https://console.cloud.google.com/apis/library/youtube.googleapis.com) 키 (선택) |

> `STUDY_LLM_API_TOKEN`이 있으면 **프록시를 우선** 사용합니다. `GEMINI_API_KEY`는 없어도 됩니다.
...
5. **Environment Variables** — `STUDY_LLM_API_TOKEN`(권장) 또는 `GEMINI_API_KEY`, `YOUTUBE_API_KEY` 등록
6. **Deploy** 클릭
